    ...
    <el-col :span="24">
      ...
      >
        ...
        <el-table-column
          ...
          width="250">
          <template slot-scope="scope">
            <el-button @click.stop="handleFinish(scope.row)" type="text" size="small" v-show="scope.row.status != '禁用'">结束</el-button>
            <el-button @click.stop="checkReceiveUser(scope.row)" type="text" size="small">已领取用户</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-col>
...
<!--    领取优惠券记录-->
    <el-dialog
      title="提示"
      :visible.sync="couponProps.dialogVisible"
      ...
      <el-col :span="24">
        <search
          style="width: 95%;margin: 10px auto"
          :search-items="searchItemsCoupon"
          @on-search="searchBySearchItem"
        ></search>
      </el-col>
      <!--    按钮和分页-->
    ...
    <el-button @click="couponProps.dialogVisible = false">返回上一页</el-button>
  </span>
    </el-dialog>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //跳转到详情页
      toDetail(row){
        this.$router.push({path: '/platformCoupon/show/' + row.id})
      },
      ...
      // 查看已领取用户
      checkReceiveUser(row){
        this.couponProps.dialogVisible = true
      },
      handleCloseCoupon(){
        this.couponProps.dialogVisible = false
      }
    },
    mounted() {
      this.search(1);
