    ...
    <el-row class="page">
      ...
      <el-col :span="24">
        <div style="width: 95%;margin: 10px auto;">
          <el-button style="background: rgb(0, 161, 108);border: none" icon="el-icon-plus"  type="primary" @click="toAdd">添加</el-button>
...
<!--              <el-button @click.stop="handleStatusChange(scope.row)" type="text" size="small">{{scope.row.status.indexOf(启用') >= 0 ? '禁用' : '启用'}}</el-button>-->
    ...
    <el-button @click="addProps.visible = false">取 消</el-button>
    <el-button type="primary" @click="addProps.visible = false">确 定</el-button>
  </span>
    </el-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleRowClick(row) {
        this.$router.push({path: '/giftPackage/show/' + row.id})
      },
