...
<!--    添加活动商品-->
    <add-activity-good
      :dialog-visible="addActivityGoodProps.visible"
      @on-dialog-close="handleClose"
      @on-dialog-confirm="handleConfirm"
    />
...
<script>
  ...
  import AddActivityGood from './addActivityGood'
  ...
  export default {
    ...
    components: {
      Search, ICreate, IEdit, AddActivityGood
    },
    methods: {
      handleEdit() {
      ...
      // 更多操作里面的添加商品
      handleAddGood(){
        this.addActivityGoodProps.visible = true;
      },
      ...
      handleClose() {
        ...
        this.addActivityGoodProps.visible = false
      },
      handleConfirm(){
        this.createProps.visible = false;
        this.editProps.visible = false;
      ...
      //跳转到详情页
      toDetail(row){
        this.$router.push({path: '/flashSaleManage/show/' + row.id})
      },
