...
<!--        <el-button style="background: rgb(0, 161, 108);border: none" icon="el-icon-plus"  type="primary" @click="toCreate">新建</el-button>-->
        <el-button style="background: rgb(0, 161, 108);border: none" type="primary" icon="el-icon-notebook-2" @click="handleMarkDeal">标记已处理</el-button>
    ...
    <el-col :span="24">
      ...
      >
        ...
        <el-table-column
          ...
          width="180">
          <template slot-scope="scope">
            <el-button @clic.stop="handleMarkDeal(scope.row)" type="text" size="small">标记已处理</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //跳转到详情页
      toDetail(row){
        // this.$router.push({path: '/opinionFeedback/haveDealShow/' + row.id})
        this.$router.push({path: '/opinionFeedback/toBeDealShow/' + row.id})
      },
      /*handleRowClick(row) {
        // this.$router.push({path: '/opinionFeedback/haveDealShow/' + row.id})
        this.$router.push({path: '/opinionFeedback/toBeDealShow/' + row.id})
